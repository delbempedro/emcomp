...
const omega  = 0.5*c/R       # Angular velocity (rad/s)
...
function retarded_time(r, t)
    """Function intended to find root."""
    function f(t_ret,t,r)
      pos = particle_position(t_ret)
      t - t_ret - sqrt((r[1] - pos[1])^2 + (r[2] - pos[2])^2)/c
    end

      t_ret = t - h  # initial guess
      for _ in 1:3
        func = f(t_ret,t,r)
        t_ret -= func*h / (f(t_ret+h,t,r) - func)
      end
      if t_ret >= t 
        println("RETURNING INACCURATE TIME.")
      end
      t_ret
end
...
function plot_field(t)
    xrange = (-10R, 10R)
    yrange = (-10R, 10R)
    npoints = 20  # Number of grid points. Not too large.
